...
import { Plus, Edit2, Trash2, X, Sliders, Filter } from '../components/Icons';
...
const AdminModifications: React.FC<AdminModificationsProps> = ({ onBack }) => {
  ...
  const fetchMods = async () => {
    ...
    try {
      ...
      alert("Errore caricamento varianti");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Eliminare definitivamente questa variante?")) {
      try {
        ...
        alert("Errore nella cancellazione. Potrebbe essere in uso in un ordine.");
      }
    }
  };
  ...
  return (
    <Layout title="Gestione Variazioni" onBack={onBack}>
      <div className="space-y-4">
        <Button fullWidth onClick={() => setEditing({ type: 'ADD', active: true, sort_order: 0 })}>
              ...
              <Card key={m.id} className={`p-4 ${!m.active ? 'opacity-50' : ''}`}>
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setEditing(m)} className="p-2 text-[#007AFF] bg-[#F2F2F7] rounded-full">
                      <Edit2 size={16} />
                    </button>
