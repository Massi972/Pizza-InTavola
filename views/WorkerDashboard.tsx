...
const WorkerDashboard: React.FC<WorkerDashboardProps> = ({ user, onLogout, onBackToAdmin }) => {
    ...
    return (
      <div className="space-y-5">
        <div className="pt-2 pb-1">
          ...
          <p className="text-base font-bold text-[#8E8E93] mt-1">
            Cosa ti prepariamo oggi?
          </p>
        </div>

        {showClosedMessage && (
          <div className="bg-[#FF3B30] text-white p-4 rounded-2xl flex items-center gap-3 shadow-lg animate-in slide-in-from-top-4 duration-300">
            ...
            <div className="flex-1">
              ...
              <p className="text-[10px] font-medium opacity-90 leading-tight">
                {availability.label.includes('OLTRE') 
                    ? `Gli ordini sono terminati alle ${settings?.cutoff_time}.`
                    : "Servizio non disponibile per la giornata odierna."
                }
              </p>
            </div>
          </div>
        ...
        {myOrder && !selectedPizza && !isEditing && (
          <Card className={`p-5 border-2 max-w-2xl mx-auto ${availability.isActive ? 'border-[#34C759] shadow-lg' : 'border-[#C6C6C8] opacity-80 grayscale'}`}>
            <div className="flex justify-between items-start gap-3 mb-5">
              <div className="flex-1">
                <p className={`text-[9px] font-black ${availability.isActive ? 'text-[#34C759]' : 'text-[#8E8E93]'} uppercase tracking-[0.2em] mb-1.5`}>Il tuo ordine per oggi</p>
              ...
              <div className="bg-[#F2F2F7] px-3 py-1.5 rounded-xl flex items-center gap-1.5 shrink-0">
            ...
            {canOrder && (
              ...
              }} variant="secondary" fullWidth className="!bg-[#F2F2F7] !py-3">
                Modifica Scelta
              </Button>
            )}
          </Card>
          ...
          <div className={`space-y-5 ${!canOrder ? 'opacity-40 pointer-events-none' : ''}`}>
            <div className="max-w-2xl mx-auto">
              <div className="relative">
                ...
                <Input 
                  ...
                  className="pl-10 !py-3 !rounded-xl border-none shadow-sm bg-white"
                  value={search}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pb-4">
              {filteredPizzas.map(pizza => (
                <Card 
                  key={pizza.id} 
                  className="p-4 cursor-pointer hover:shadow-md active:scale-[0.97] transition-all flex flex-col h-full border border-transparent hover:border-[#007AFF]/10"
                ...
                >
                  <div className="flex justify-between items-start mb-1.5">
                    ...
                    {pizza.isVegetarian && (
                      <span className="bg-green-100 text-green-700 text-[8px] px-1.5 py-0.5 rounded-full font-black tracking-wider shrink-0">VEG</span>
                    )}
                  </div>
                  <p className="text-[11px] text-[#8E8E93] leading-snug flex-1 italic line-clamp-2">
  ...
  return (
    <Layout 
      title={activeTab === 'menu' ? 'InTavola Staff' : 'Profilo'}
    ...
    >
      {showSuccess && (
        <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center p-6 bg-white/95 backdrop-blur-2xl animate-in fade-in duration-300">
          <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mb-5 animate-in zoom-in duration-500 shadow-md">
      ...
      {errorMessage && (
        <div className="fixed top-[max(60px,env(safe-area-inset-top))] left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-sm p-4 rounded-xl shadow-2xl bg-[#FF3B30] text-white flex items-center gap-3 animate-in slide-in-from-top-10 duration-500">
          <AlertCircle size={20} className="shrink-0" />
          <p className="font-bold text-[11px] flex-1 leading-tight">{errorMessage}</p>
      ...
      {activeTab === 'menu' ? renderMenu() : (
        <div className="max-w-2xl mx-auto space-y-6 animate-in slide-in-from-right duration-300">
           <Card className="p-6 text-center bg-white shadow-md">
            <div className="w-16 h-16 bg-gradient-to-br from-[#007AFF] to-[#5856D6] rounded-full flex items-center justify-center text-white mx-auto shadow-md mb-4">
              <UserIcon size={32} />
            </div>
          ...
          <div className="grid grid-cols-1 gap-5">
            <section className="space-y-2">
              <p className="text-[9px] font-black text-[#8E8E93] uppercase tracking-widest pl-1">Sicurezza e Dispositivo</p>
                ...
                {isBioSupported ? (
                  <div className="p-4 flex items-center justify-between hover:bg-[#F2F2F7] transition-colors cursor-pointer" onClick={() => !biometricEnabled && registerBiometrics(user.id, user.firstName).then(s => s && setBiometricEnabled(true))}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs font-bold">Accesso Biometrico</p>
                        <p className="text-[9px] text-[#8E8E93] font-bold tracking-tight">Face ID / Touch ID</p>
                      </div>
                    </div>
                    <div className={`w-10 h-6 rounded-full transition-colors relative flex items-center px-1 cursor-pointer ${
                  ...
                  <div className="p-4 flex items-center gap-3 opacity-50 grayscale bg-gray-50">
                    ...
                    <p className="text-[10px] font-bold">Biometria non disponibile su questo browser</p>
                  </div>
                )}
              </Card>
            </section>
              ...
              <Card className="divide-y divide-[#F2F2F7]">
                {isManagement && onBackToAdmin && (
                  <button onClick={onBackToAdmin} className="w-full p-4 flex justify-between items-center text-[#5856D6] font-bold active:bg-[#F2F2F7] transition-all">
                    <div className="flex items-center gap-3">
                      <div className="p-1.5 bg-[#F2F2F7] text-[#5856D6] rounded-lg">
                        <Sliders size={16} />
                      </div>
                ...
                <button onClick={onLogout} className="w-full p-4 flex justify-between items-center text-[#FF3B30] font-bold active:bg-red-50 transition-all">
                   <div className="flex items-center gap-3">
                      <div className="p-1.5 bg-red-50 text-[#FF3B30] rounded-lg">
                        <LogOut size={16} />
                      </div>
      ...
      {/* Navigazione iOS-style protetta dalla barra home */}
      <nav className="fixed bottom-0 left-0 right-0 ios-blur border-t border-[#C6C6C8] pt-2 pb-[calc(0.75rem+env(safe-area-inset-bottom))] flex justify-center z-40">
        <div className="w-full max-w-lg px-10 flex justify-between items-center">
          <button 
            ...
            className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'menu' ? 'text-[#007AFF] font-bold' : 'text-[#8E8E93] opacity-60'}`}
          >
            <PizzaIcon size={22} />
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'settings' ? 'text-[#007AFF] font-bold' : 'text-[#8E8E93] opacity-60'}`}
          >
            <Settings size={22} />
      ...
      {/* Modal selezione pizza adattata per Safe Area */}
      {selectedPizza && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end items-center overflow-hidden">
          ...
          <div className="relative bg-[#F2F2F7] w-full max-w-lg rounded-t-[28px] p-5 space-y-5 shadow-2xl animate-in slide-in-from-bottom duration-300 overflow-y-auto max-h-[90dvh] pb-[calc(2rem+env(safe-area-inset-bottom))]">
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-5">
                <section className="space-y-2">
                  ...
                  <div className="bg-white rounded-xl shadow-sm overflow-hidden divide-y divide-[#F2F2F7]">
                    {addOptions.length === 0 && <p className="p-3 text-[10px] text-center text-gray-400 italic">Nessun ingrediente extra</p>}
                    {addOptions.map(opt => (
                      <button 
                        key={opt.id} 
                        onClick={() => handleToggleAdd(opt.id)}
                        className="w-full flex items-center justify-between p-3 active:bg-[#F2F2F7] transition-colors"
                      >
                        ...
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                          ...
                          {selectedAddIds.includes(opt.id) && <Check size={12} className="text-white" strokeWidth={3} />}
                        </div>
                      </button>
                    ))}
                  </div>
                    ...
                    {removeOptions.map(opt => (
                      ...
                      >
                        ...
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                          ...
                          {selectedRemoveIds.includes(opt.id) && <Check size={12} className="text-white" strokeWidth={3} />}
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              <Button fullWidth onClick={() => setShowRecap(true)} disabled={submitting} className="!py-3.5 !text-base">
                Conferma Ordine
              </Button>
            </div>
          </div>
