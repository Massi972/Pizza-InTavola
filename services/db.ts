...
const SUPABASE_URL = 'https://akifadjnpedvwesxzbsw.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFraWZhZGpucGVkdndlc3h6YnN3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzA4MTUzMTAsImV4cCI6MjA4NjM5MTMxMH0.5vHJku7L7ruZApORXxJZaEJgC39EWnApyM8vbcQNKko';
...
class DB {
  ...
  async getMasterCode(): Promise<string> {
    ...
    if (error || !data) return 'PIZZA2025'; // Default fallback
  ...
  async getCurrentDay(): Promise<Day | null> {
    ...
    if (error || !data) return null;
    return { id: data.id, date: data.date, status: data.status, openedAt: data.opened_at, closedAt: data.closed_at };
  }
  ...
  async openDay(): Promise<Day> {
    ...
    const { data, error } = await supabase
      .from('days')
      .upsert({ date: today, status: 'OPEN', opened_at: new Date().toISOString() }, { onConflict: 'date' })
      .select().single();
    if (error) throw error;
    return { id: data.id, date: data.date, status: data.status, openedAt: data.opened_at, closedAt: data.closed_at };
  }
  ...
  async closeDay(): Promise<void> {
    const today = new Date().toISOString().split('T')[0];
    const { error } = await supabase.from('days').update({ status: 'CLOSED', closed_at: new Date().toISOString() }).eq('date', today);
    if (error) throw error;
  }
  ...
  async getUserOrderToday(userId: string): Promise<Order | null> {
    ...
    if (error || !data) return null;
    return { id: data.id, dayId: data.day_id, userId: data.user_id, pizzaId: data.pizza_id, slotTime: data.slot_time as SlotTime, note: data.note, createdAt: data.created_at, updatedAt: data.updated_at };
  }

  async saveOrder(order: Partial<Order>): Promise<void> {
    const payload = { day_id: order.dayId, user_id: order.userId, pizza_id: order.pizzaId, slot_time: order.slotTime, note: order.note, updated_at: new Date().toISOString() };
